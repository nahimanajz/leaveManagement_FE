...
import { LeaveType, Employee, LeaveRequest, Department } from '@/types/leaveTypes';

export const leaveTypes: LeaveType[] = [
  {
    id: "pto",
    name: "Personal Time Off",
    description: "Annual leave for personal use",
    color: "#8B5CF6",
    defaultDays: 20,
    isActive: true,
...
const generateLeaveRequests = (): LeaveRequest[] => {
  ...
  employees.forEach(employee => {
    ...
    for (let i = 0; i < requestCount; i++) {
      ...
      const leaveTypeIds = ["pto", "pto", "pto", "sick", "compassionate", "maternity"];
      ...
      if (leaveTypeId === "maternity" && employee.leaveBalances["maternity"] === 0) {
        continue;
      }
      
      // Set request date to 1-14 days before start date
      const requestDate = new Date(startDate);
      requestDate.setDate(startDate.getDate() - (1 + Math.floor(Math.random() * 14)));
      ...
      requests.push({
        ...
        approvedBy: status === 'approved' ? 
          departments.find(d => d.id === employee.department)?.managerId : undefined,
